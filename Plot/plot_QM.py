...
class QM0_t(object):
    ...
    def plot_all(self):
        ...
        for irep, qmF in enumerate(self.all_QM_0_data):
            ...
            for iatom in range(len(qmMag[0])):
                QM0_t.plotAx.plot(timesteps,
                                   qmMag[:, iatom],
                                   '-',
...
#                Qlk_t._set_Qlk_t_control()
    ...
    def _check_settings_Qlk_t(label):
        if label == 'X':
            Qlk_t.X = not Qlk_t.X
            for ln in Qlk_t.Xlines:
                ln.set_visible(Qlk_t.X)
        if label == 'Y':
            Qlk_t.Y = not Qlk_t.Y
            for ln in Qlk_t.Ylines:
                ln.set_visible(Qlk_t.Y)
        if label == 'Z':
            Qlk_t.Z = not Qlk_t.Z
            for ln in Qlk_t.Zlines:
                ln.set_visible(Qlk_t.Z)
        if label == 'Magnitude':
            Qlk_t.Mag = not Qlk_t.Mag
            for ln in Qlk_t.Maglines:
                ln.set_visible(Qlk_t.Mag)
        plt.draw()
    ...
    def _turn_on_atoms(Min, Max, type_switch='opposite'):
        ...
        poss_lines = [Qlk_t.Xlines, Qlk_t.Ylines, Qlk_t.Zlines, Qlk_t.Maglines]
        all_switches = [Qlk_t.X, Qlk_t.Y, Qlk_t.Z, Qlk_t.Mag]
        ...
        for iat, linelist in enumerate(Qlk_t.all_rep_lines[Min: Max]):
            for ln in linelist:
                if type_switch == 'opposite':
    ...
    def _submit_text(text):
        ...
        """
        if text == 'all':
            Qlk_t._turn_on_atoms(0, len(Qlk_t.Xlines), True)
        else:
            text = text.split(',')
            for item in text:
                minmax = item.split('-')
                if len(minmax) == 2:
                    ...
                    try:
                        ...
                        print("Tried to convert '%s' to ints but" +
                              " couldn't" % item)
                if Max > len(Qlk_t.Xlines):
                    Max = len(Qlk_t.Xlines)
                Qlk_t._turn_on_atoms(0, len(Qlk_t.Xlines), False)  # Turn all off
                Qlk_t._turn_on_atoms(Min, Max, True)  # Turn the correct ones on

    # Will set the control panel for Qlk_t graph
    @staticmethod
    def _set_Qlk_t_control():
        Qlk_t.check_Qlk_t = CheckButtons(Qlk_t.widget_ax[0],
                                        ('X', 'Y', 'Z', 'Magnitude'),
...
class QM_R(object):
    ...
    def _match_timesteps_Qlk_pos(self, all_Qlk_data, RI0_data):
        """
        Will find the Qlk data and the posistion data that share the same 
        timesteps.
        """
        for Qlk_rep, pos_rep in zip(all_Qlk_data, RI0_data):
            Qlk_data = all_Qlk_data[Qlk_rep]
            pos_data = RI0_data[pos_rep]
            ...
            RI0_data[pos_rep] = pos_data
        
        return all_Qlk_data, RI0_data
    ...
    def _plot_qm_vs_pos_all_reps(self, istep):
        ...
        all_QM_data, all_pos = self._match_timesteps_Qlk_pos(self.all_Qlk_data, self.RI0_data)
        ...
        for irep in range(len(all_QM_data)):
            for iatom in range(natom):
                ...
                ln, = QM_R.plotAx.plot(single_rep_pos[istep,iatom, idim], 
                                  Qlk_data[istep,0], 
                                  '.', color=self.colors[iatom])
