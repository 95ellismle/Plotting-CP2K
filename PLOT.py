...
class LoadData(object):
    """
    Will load all data in the 
    """
    def __init__(self, folder, reps, plot_params='all'):
        self.folder = folder
        self.reps = reps
    ...
    def load_all_di_coeffs(self):
        ...
        """
        if any(j in self.plot_params for j in ('norm','|u|^2')):
...
class Plot(LoadData):
    ...
    def __init__(self, plot_params, folder, reps, avg_on=True, plot_all_reps=True):
        self.plot_params = plot_params
        self._correct_plot_params()
        LoadData.__init__(self, folder, reps, self.plot_params)
        self.reps = reps
        self.avg_on = avg_on
        self.plot_all_reps = plot_all_reps
    ...
    def _get_alpha(self):
        ...
        alphas = {1: 1, 2: 0.5, 3: 0.4, 10: 0.2, 50: 0.1, 100: 0.075, 200: 0.05, 500: 0.01, 10000:0}
        self.all_alphas = {}
    ...
    def _average_data(self):
        ...
        """
        if self.avg_on:
            if any(j in self.plot_params for j in ('norm','|u|^2')) and list(self.all_Dcoeff_data.keys())[0] != 0:
    ...
    def _plot_norm(self):
        ...
        if 'norm' in self.plot_params:
            ...
            if self.avg_on:
                ...
                ax.plot(timesteps, norms, lw=2, color='r')
            if self.plot_all_reps:
                for Dfilename in self.all_Dcoeff_data:
                    coeffs, cols, timesteps, pops = self.all_Dcoeff_data[Dfilename]
                    norms = np.sum(pops, axis=1)
                    ax.plot(timesteps, norms, alpha=self.alpha, color='r', lw=0.7)
            ax.set_ylabel(r"$\sum_l |u_l|^2$")
    ...
    def _plot_adiab_states(self):
        ...
        if 'adiab_states' in self.plot_params:
            ...
            ax.set_ylabel("Adiabatic Energy")
...
folder = fold.make_fold_abs('../../200Rep_3mol') #The folder to look in for the data
p = Plot(['norm'], folder, 'all', plot_all_reps=True)